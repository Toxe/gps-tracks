...
def test_login(client, example_users):
    r = client.post("/auth/login", json={"email": "user1@example.com", "password": "password1"})
...
def test_login_unknown_user(client, example_users):
    r = client.post("/auth/login", json={"email": "unknown@example.com", "password": "password1"})
...
def test_login_with_wrong_password(client, example_users):
    r = client.post("/auth/login", json={"email": "user1@example.com", "password": "wrong"})
    assert r.status_code == 401
    assert r.is_json
    data = r.get_json()
    assert data.get("message") == "Login email address or password missing."
...
def test_cannot_call_protected_api_with_refresh_token(client, auth, example_users):
    auth.login("user1@example.com", "password1")
    r = client.delete("/api/users/1", headers={"Authorization": "Bearer {}".format(auth.refresh_token)})
    assert r.status_code == 422
    assert r.is_json
...
def test_logout_access_token(client, auth, example_users):
    ...
    # no longer logged in
    r = client.delete("/api/users/1", headers={"Authorization": "Bearer {}".format(auth.access_token)})
    assert r.status_code == 401
    assert r.is_json
