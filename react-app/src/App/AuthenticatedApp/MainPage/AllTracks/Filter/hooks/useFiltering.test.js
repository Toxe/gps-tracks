...
describe("useFiltering()", () => {
    describe("With tracks, checking available activities", () => {
        test('When having only activity 0, listAvailableActivities should return activities list ["0"]', () => {
            const tracks = [
            ...
            const availableActivities = result.current.listAvailableActivities(tracks);

            expect(availableActivities).toBeArrayOfStrings();
            expect(availableActivities).toHaveLength(1);
            expect(availableActivities).toEqual(["0"]);
        });

        test('When having only activity 1, listAvailableActivities should return activities list ["1"]', () => {
            const tracks = [
            ...
            expect(availableActivities).toBeArrayOfStrings();
            expect(availableActivities).toHaveLength(1);
            expect(availableActivities).toEqual(["1"]);
        });

        test('When having activities 0 and 1, listAvailableActivities should return activities list ["0", "1"]', () => {
            const tracks = [
                { activity_mode: 1, time_start: "2017-06-02T13:18:14.999000" },
                { activity_mode: 0, time_start: "2018-06-02T13:18:14.999000" },
            ...
            expect(availableActivities).toBeArrayOfStrings();
            expect(availableActivities).toHaveLength(2);
            expect(availableActivities).toContainValues(["0", "1"]);
        });
    });

    describe("With tracks, checking available years", () => {
        test('When having only year 2016, listAvailableYears should return years list ["2016"]', () => {
            const tracks = [
            ...
            const availableYears = result.current.listAvailableYears(tracks);

            expect(availableYears).toBeArrayOfStrings();
            expect(availableYears).toHaveLength(1);
            expect(availableYears).toEqual(["2016"]);
        });

        test('When having years 2016 and 2017, listAvailableYears should return years list ["2016", "2017"]', () => {
            const tracks = [
            ...
            expect(availableYears).toContainValues(["2016", "2017"]);
        });
    });
    ...
    describe('With tracks and activityFilter == "" and yearFilter == ""', () => {
        test("When calling filterTracks, should return all tracks", () => {
            ...
            const filteredTracks = result.current.filterTracks(tracks);
    ...
    describe("Without tracks, checking filterTracks returns", () => {
        ...
        test('When "tracks" is null, filterTracks should return empty array', () => {
            ...
            expect(result.current.filterTracks(null)).toBeEmptyArray();
        });
    });
    ...
    describe("Without tracks, checking available activities and years", () => {
        test('When "tracks" is empty array, available activities and years should be empty arrays', () => {
            ...
            expect(result.current.listAvailableActivities([])).toBeEmptyArray();
            expect(result.current.listAvailableYears([])).toBeEmptyArray();
        });
        ...
        test('When "tracks" is null, available activities and years should be empty arrays', () => {
            ...
            expect(result.current.listAvailableActivities(null)).toBeEmptyArray();
            expect(result.current.listAvailableYears(null)).toBeEmptyArray();
        });
    });
